...
usage() {
    ...
    echo "Script to build libusermetrics. If BUILD_TYPE is not specified, it defaults to \"Debug\".\n" >&2
    echo "OPTIONS:" >&2
...
(
  cd $BUILDDIR
  cmake "$SOURCEDIR" -G "$GENERATOR" -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DBUILD_DOXYGEN=YES
  $BUILD_COMMAND
)
